import { UpdateEventDto, WeddingEvent } from "../../types/api.generated";
...
export const EventApi = {
  ...
  findWeddingEvents: async (weddingId: string): Promise<WeddingEvent[]> => {
    const response = await api.get<WeddingEvent[]>(
      `/events/weddings/${weddingId}`,
    );
    return response.data;
  },
