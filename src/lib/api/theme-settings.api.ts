...
export const ThemeSettingsApi = {
  ...
  findByWeddingId: async (weddingID: string): Promise<ThemeSettings> => {
    const response = await api.get<ThemeSettings>(
      `/theme-settings/weddings/${weddingID}`,
    );
    return response.data;
  },
  ...
  update: async (
    id: string,
    data: UpdateThemeSettingsDto,
  ): Promise<ThemeSettings> => {
    const response = await api.patch<ThemeSettings>(
      `/theme-settings/${id}`,
      data,
    );
    return response.data;
