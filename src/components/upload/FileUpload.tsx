...
export const FileUpload = ({
  ...
  const { mutateAsync: uploadSingle, isPending: isUploadingSingle } =
    useUploadSingle();
  const { mutateAsync: uploadBatch, isPending: isUploadingBatch } =
    useUploadBatch();
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-[#8B7355] mt-2">
          ...
          {mode === "single" ? "Single file" : "Multiple files"}
        </p>
      </div>
      ...
      {uploadedUrls.length === 0 && !isUploading && (
        <div className="border-2 border-dashed border-[#E8DDD5] rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-[#8B7355]">No images uploaded yet</p>
        </div>
      )}
    </div>
  );
